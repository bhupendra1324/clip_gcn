...
class CLIP_GCN_Model(nn.Module):
    def __init__(self):
        ...
        self.text_encoder = GCN(n = 6, edges=edge_index, in_channels=512, out_channels=512, hidden_layers='d256,d').to(device)
...
# MODEL_NAME = "CLIP+GCN_Model_NewLoss.pth"
...
MODEL_PATH = Path("models")
MODEL_NAME = "CLIP+GCN_Model_newgcn.pth"
MODEL_SAVE_PATH = MODEL_PATH / MODEL_NAME
...
savetxt('New_GCN_features_train.csv', features, delimiter=',')
savetxt('New_GCN_labels_train.csv', labels, delimiter=',')
...
print('Testing the model for tsne on the unlabelled data')
...
labels = np.array([])
with torch.inference_mode():
  for batch in tqdm(test_dataloader, desc='Running the model inference'):
    img, label = batch
    features = np.vstack([features, loaded_model(batch).cpu().numpy()]) if features.shape[0] > 0 else loaded_model(batch).cpu().numpy()
    labels = np.append(labels, label.cpu().numpy())
      
savetxt('New_GCN_features.csv', features, delimiter=',')
savetxt('New_GCN_labels.csv', labels, delimiter=',')

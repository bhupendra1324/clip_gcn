...
class Image_Encoder(nn.Module):
  def __init__(self):
    super().__init__()
    self.pre_model = timm.create_model('vit_base_patch32_224', pretrained=True).to(device)
    # Remove the classification head of the model
    self.pre_model.head = torch.nn.Identity()
...
MODEL_PATH = Path("models")
MODEL_NAME = "CLIP+GCN_Model_newmodel.pth"
MODEL_SAVE_PATH = MODEL_PATH / MODEL_NAME
